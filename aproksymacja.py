...
def f(points_x, points_y):
    ...
    sum=0
    for i in range(n):
        sum +=  (a*points_x[i]**3 + b*points_x[i]**2 + c*points_x[i] + d - points_y[i])**2
...
def aprox(f):
    ...
    # return sp.simplify(solutions[a]*x**3 + solutions[b]*x**2 + solutions[c]**x + solutions[d])
    return f"Approximation function: {solutions[a]}*x^3 + {solutions[b]}*x^2 + {solutions[c]}*x + {solutions[d]}"
